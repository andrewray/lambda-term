...
let draw_frame_labelled ctx rect ?style ?label_style ?(alignment=H_align_left) label connection = 
  draw_frame ctx rect ?style connection;
  try 
    let ctx = sub ctx { row1=0; row2=1; col1=1; col2=rect.col2-1 } in
    draw_string_aligned ctx ?style:label_style rect.row1 alignment label
  with Out_of_bounds -> ()
